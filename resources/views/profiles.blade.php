...
<a href="{{route('adduser')}}" class="btn btn-primary">افزودن کاربر جدید</a>
<a class="btn  btn-info" href="{{ url('excel')}}" >خروجی اکسل</a>
<table class="table">
  <thead>
    <tr>
        <td>شناسه </td>
    ...
    <tr>
        ...
        <td>{{$cat->password}} </td>
        <td>
        <?php if($cat->role==3)echo 'کاربر' ?>
        <?php if($cat->role==2)echo 'ادمین میانی' ?>
        <?php if($cat->role==1)echo 'ادمین کل' ?>
         </td>
        <td>{{$cat->phone}} </td>
        <td>{{$cat->tnphone}} </td>
        <td>{{$cat->buytype}}</td> 
        <td><a class="btn btn-danger"href="{{route('destroy',$cat->id)}}"
        onclick="return confirm('آیتم حذف شود')">حذف</a></td>
        <td><a class="btn btn-success" href="{{route('edit',$cat->id)}}" >ویرایش</a></td>
